...
- Node.js 앱 하나 돌리려고 Ubuntu 커널과 시스템 도구까지 올려야 함
- OS 부팅 과정도 거쳐야 해서 시작에 **수십 초~수 분** 소요

### 컨테이너가 가벼운 이유

- 커널은 이미 호스트에 있으니 또 설치할 필요 없음
...
결론: 생노가다로 aws에서 다 새팅하는거 보다 docker 딸깍이 많이 많이 편함
---
...
> DB는 "감싸는" 단계가 없다. 해당 팀이 이미 이미지를 만들어 Docker Hub에 올려놨기 때문. 걍 빌드하면 됨
...
- **Containers 탭**: 실행 중이거나 멈춘 컨테이너 목록
- **Volumes 탭**: 데이터 저장소 관리

---

## Dockerfile 작성법 (uv 기준임)
...
docker compose up -d --build   # 코드 수정 후 전체 재빌드하면서 시작
docker compose down            # 전부 종료 (데이터 유지)
docker compose down -v         # 전부 종료 + 볼륨(DB 데이터)까지 삭제 (이거 잘못했다 개고생한다)
...
근데 그 데이터 저장 그거만 바꾸면 18로도 되서 18로 다시 함
...
# 느낀거
개념은 약간 아리송한데 막상해보니 생각보다 굉장히 쉬운 줄 알았는데 DB 관련해서 막 로컬,도커 막 엇갈리고 포트 뭐 엇갈리고 그런부분이 좀 많이 그지같았음.
일단 compose 하면 딸깍 한번에 서버,db들 다켜지는게 굉장히 신선했음. 그리고 진짜 간편하고 쉽긴 하더라. 개발 초기에 미리 도커 따로 한다음에 하는게 좀 더 편할 듯.
나중에 막 db 따로 해야되고 뭐 아무튼 좀 뭐시기한 일이 좀 일어나긴 함.
그래도 생각보다 쉽게 사용법 익힐 수 있었다~. 이제 도커 더 익숙해지면 aws 공부 좀 해야겠음.
